...
class InputDrivenObservations(Observations):
    ...
    def __init__(self, K, D, M=0, C=2, prior_mean = 0, prior_sigma=5):
        """
        ...
        self.posterior_means = np.zeros((K,M))
        self.posterior_covs = np.zeros((K,M,M))
    ...
    def calculate_logits(self, input):
        ...
        Wk = np.transpose(np.vstack([np.zeros((1, Wk_tranpose.shape[1], Wk_tranpose.shape[2])), Wk_tranpose]),
                          (1, 0, 2))
    ...
    def samplefromposterior(self, datas, inputs, zs):
        """
            Sample from the conditional of observation parameters for Gibbs sampling
        """
        # To store mean of the estimated posterior over weights for each state
        weights_means = np.empty((self.K,self.M))
        ...
        for k in range(self.K):
            ...
            Yk = datas[zs == k]
            Yk = np.reshape(Yk, (Yk.shape[0], self.D))
            Xk = inputs[zs == k]
            Xk = np.reshape(Xk, (Xk.shape[0], self.M)) 
            # Previous weight
            weight_old = self.Wk[k]
            ...
            if accept==1:
                weights_sampledk = weight_proposed
            else:
                weights_sampledk = weight_old
            # Ensure this is (1,C-1,M)
            self.Wk[k] = np.reshape(weights_sampledk, (1,self.C-1, self.M))
            # Just returning the sampled weight cause we no more care about the mean of the proposal
            weights_means[k] = weights_sampledk

        return weights_means


    def polyagammasample(self, datas, inputs, zs):
        """
            Sample from the conditional of observation parameters using polya-gamma augmentation
        """
        # To store mean of the estimated posterior over weights for each state
        weights_means = np.empty((self.K,self.M))
        zs = np.concatenate(zs)
        ...
        # Inverse prior covariance, will be used later
        inv_prior_covariance = (1/(self.prior_sigma**2))*np.eye(self.M,self.M)
        # Prior mean
        prior_w0 = self.prior_mean*np.ones((self.M))
        ...
        for k in range(self.K):
            ...
            # Initialize the polygamma sampler
            N_samples = 6
            ...
            for i in range(0, N_samples):
                ...
                for timestep in range(N):
                    omegas[timestep] = pg.pgdraw(1, XTw[timestep].ravel())

                # Sample weight given omega, Y, X from its Gaussian conditional
                z = (Yk - 0.5).ravel()/omegas
                V = np.linalg.inv(Xk.T@np.diag(omegas)@Xk + inv_prior_covariance)
                m = V @ (Xk.T@np.diag(omegas)@z + inv_prior_covariance@prior_w0)
                weights_sampledk = st.multivariate_normal.rvs(mean = m, cov=V)   
    ...
    def compute_expected_loglikelihood(self, datas, inputs, means, covs, samples=10):
        """ Compute expected likelihood of the data using samples from a variational gaussian over GLM weights"""
        Ts = [data.shape[0] for data in datas]
        K = self.K
        M = inputs[0].shape[1]
        sampled_logLs = [np.zeros((T,self.K)) for T in Ts]
        sampled_weights = np.zeros((samples, K, M))

        for state in range(K):
            sampled_weights[:,state,:] = np.reshape(st.multivariate_normal.rvs(means[state], covs[state], size=(samples,)), (samples,M))

        for n in range(samples):
            self.Wk = np.reshape(sampled_weights[n], (K,1,M))
            # Obtain observation Likelihoods (TXKXC)
            log_likes = [self.calculate_logits(inpt) for inpt in inputs]
            # Extract only emission potentials
            for sess in range(len(Ts)):
                ind_0 = np.argwhere(datas[sess]==0).ravel()
                ind_1 = np.argwhere(datas[sess]==1).ravel()
                sampled_logLs_perweight = np.empty((Ts[sess], K))
                sampled_logLs_perweight[ind_0] = log_likes[sess][ind_0,:,0]
                sampled_logLs_perweight[ind_1] = log_likes[sess][ind_1,:,1]
                sampled_logLs[sess] = sampled_logLs[sess] + sampled_logLs_perweight
                # for t in range(Ts[sess]):
                    #  sampled_logLs[sess][t,:] = sampled_logLs[sess][t,:] + log_likes[sess][t,:,int(datas[sess][t].ravel())]   

        sampled_logLs = [sampled_logLs[sess]/samples for sess in range(len(Ts))]
        return sampled_logLs

    def expected_laplace_approximation(self, qzt_ks, datas, inputs, old_weights):
        """
        Laplace Approximation for variational update
        """
        # Inputs should be (TXM)
        T = inputs.shape[0] 
        weights = old_weights.T

        # Sample weights from prior_covariance
        prior_covariance = (self.prior_sigma**2)*np.eye(self.M,self.M)
        prior_w0 = self.prior_mean*np.ones((self.M))

        def expecteddnegP(weights, datas, inputs, qzt_ks):
            """
            Function to compute derivative of negative log posterior of bernoulli GLM
            """
            inv_prior_covariance = (1/(self.prior_sigma**2))*np.eye(self.M,self.M)
            prior_w0 = self.prior_mean*np.ones((self.M))
            return expecteddnegL(weights, datas, inputs, qzt_ks) + inv_prior_covariance@(weights-prior_w0)

        def expectedHessnegP(weights, datas, inputs, qzt_ks):
            """
            Function to compute hessian of negative log posterior of bernoulli GLM
            """
            inv_prior_covariance = (1/(self.prior_sigma**2))*np.eye(self.M,self.M)
            return expectedHessnegL(weights, datas, inputs, qzt_ks) + inv_prior_covariance

        # MAP inference for weights
        def w_map(weights, datas, inputs, qzt_ks):
            # Compute projection of inputs onto GLM weights for each class
            wTx = inputs@weights
            
            # Evaluating softplus and its derivatives
            f, df, ddf = softplus(wTx)
            expectednegL = -(qzt_ks[:,np.newaxis]*datas).T@wTx + np.sum(qzt_ks*f)
            # Negative LP
            expectednegP = expectednegL - st.multivariate_normal.logpdf(weights, mean=prior_w0, cov=prior_covariance)
            return (expectednegP, expecteddnegP(weights, datas, inputs, qzt_ks))

        Results = optimize.minimize(w_map, x0=weights, method='trust-exact', jac = True, hess = expectedHessnegP, args = (datas, inputs, qzt_ks))
        weights_MAP = Results['x']
        # This might be the bottleneck, when increasing input dimensionality/ See if Lewi's idea applies here
        covariance = np.linalg.inv(expectedHessnegP(weights_MAP, datas, inputs, qzt_ks))
        return weights_MAP, covariance



    def variationalupdate(self, qzts, datas, inputs, weights):
        """ Compute mean field variational update for model parameters using Laplace approximation"""
        datas = np.concatenate(datas)
        inputs = np.concatenate(inputs)

        K = weights.shape[0]
        M = self.M
        posterior_means = np.empty((K,M))
        posterior_covs = np.empty((K,M, M))

        for k in range(K):
            mean_k, cov_k =  self.expected_laplace_approximation(qzts[:,k], datas, inputs, weights[k])
            posterior_means[k] = mean_k
            posterior_covs[k] = cov_k

        self.posterior_covs = posterior_covs
