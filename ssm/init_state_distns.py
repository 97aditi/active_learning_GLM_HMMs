...
class InitialStateDistribution(object):
    ...
    def samplefromposterior(self, zs):
        """"
            Sample from full conditional of the mixing weight
        """
        #Count of discrete states at step 0
        N0 = np.zeros(self.K, dtype='int')
        z0s = [z[0] for z in zs]
        unique, counts = np.unique(z0s, return_counts=True)
        for k in range(self.K):
            if k<len(counts):
                N0[k] = counts[k]
        # Sample from conditional of pi_0
        # pi ~ Dir(alpha(0) + n)
        pi0 = np.random.dirichlet(self.prior_alpha+N0)
