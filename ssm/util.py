...
def permute_params(posterior_alpha_0, posterior_alpha_Ps, posterior_means, posterior_covs, perm=None):
    if perm is None:
        # this is only useful for our simulated 3-state model setup
        state_1 = np.argmax(posterior_means[:,0])
        state_2 = np.argmin(posterior_means[:,1])
        state_3 = np.setdiff1d(np.arange(3) , np.array([state_1, state_2]))[0]
        perm = np.array([state_1, state_2, state_3])
    
    posterior_alpha_0 = posterior_alpha_0[perm].copy()
    posterior_alpha_Ps = posterior_alpha_Ps[np.ix_(perm, perm)].copy()
    posterior_means = posterior_means[perm,:].copy()
    posterior_covs = posterior_covs[perm,:].copy()
    return posterior_alpha_0, posterior_alpha_Ps, posterior_means, posterior_covs
